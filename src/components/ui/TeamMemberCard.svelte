...
>
	<div class="relative mb-6">
		<Image
			src={member.image}
			alt={`${member.name}`}
			class="border-secondary-100 h-32 w-32 rounded-full border-4 transition-transform duration-300 group-hover:scale-110"
			width={128}
			height={128}
		/>
	...
	<div class="text-secondary-500 mt-6 flex items-center justify-center gap-6 text-sm">
		<div class="flex items-center gap-2">
			...
			<span class="text-xs">{m['team.repos']()}</span>
		</div>
		<div class="flex items-center gap-2">
			<svg
				xmlns="http://www.w3.org/2000/svg"
			...
			>
				<path
					d="M16 4c0-1.11.89-2 2-2s2 .89 2 2-.89 2-2 2-2-.89-2-2zm4 18v-6h2.5l-2.54-7.63c-.34-1.02-1.28-1.74-2.34-1.74-.38 0-.74.1-1.06.28L13.5 8.5c-.5.5-.5 1.29 0 1.79l2.32 2.32c.39.39.9.59 1.41.59H18v8h2z"
				/>
			</svg>
			<span class="font-medium">{member.followers}</span>
			<span class="text-xs">{m['team.followers']()}</span>
		</div>
	</div>
</div>
